...
{	  
	//set_leds(0x0F);
	process_data();						   //Upro fuer Filter
		
	DMA1->IFCR &= 0x00000000;			  //flags löschen , pending bit reseten
	DMA1->IFCR |= 0xFFFFFFFF;
	NVIC->ICPR[14/32]  |=  (0x01<<(14%32));
	
	//set_leds(0x00);						//Enablen: DMA2_CH2			richtig
...
{
	set_leds(0x55);
	//DMA_Cmd(DMA2_Channel2, DISABLE);												//Enablen: DMA2_CH2			richtig
  
	DMA2_Channel2->CNDTR = 0x2;

	DMA2->IFCR &= 0x00000000;
	DMA2->IFCR |= 0xFFFFFFFF;
	NVIC->ICPR[57/32]  |=  (0x01<<(57%32));
