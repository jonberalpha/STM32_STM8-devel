...
{	
	...
	TIM1->CR1 &= 0x00; // Auswahl des Timer Modus: Upcounter --> Zaehlt: von 0 bis zum Wert des Autoreload-Registers
	...
	TIM1->ARR = 52801; //Auto-Reload Wert = Maximaler Zaehlerstand des Upcounters
	TIM1->RCR &= 0x00; //Repetition Counter deaktivieren
	
	/*--------------------- Interrupt Timer 1 konfigurieren --------------------*/
	TIM1->DIER |= 0x01; //enable Interrupt bei einem UEV (UEberlauf/Unterlauf)
	TIM1->SR &= ~0x01; //Pending-Bit loeschen
	NVIC_init(TIM1_UP_IRQn,2); //Enable Timer 1 Update Interrupt, Priority 3
	
	/*---------------------------- Enable Timer 1 ------------------------------*/
	TIM1->CR1 |= 0x01; //Enable the Timer 1
}
...
/*   CC3 CH4(PB1) als Output Capture verwendet													      */
/******************************************************************************/
...
{	
	...
	TIM3->PSC = 150; //Wert des Prescalers (Taktverminderung); 1ms
	TIM3->ARR = 52801; //Auto-Reload Wert = Maximaler Zaehlerstand des Upcounters
	TIM3->RCR |= 0x00; //Repetition Counter deaktivieren
	
	/*--------------------- Interrupt Timer 3 konfigurieren --------------------*/
	TIM3->DIER = 0x01; //enable Interrupt bei einem UEV (UEberlauf / Unterlauf)
	TIM3->SR &= ~0x01; //Pending-Bit loeschen
	
	/*--------------------- Configure Channel 4(PB1) as OC ---------------------*/
	TIM3->CCR4 = 10000; //1/4 von ARR-Wert=13200
	TIM3->CCMR2 |= 0x7800; //CC4S=00, OC4PE=1, OC4M=111
	TIM3->CCER |= 0x01000; //Enable CH4 als Output Capture Compare
	
	/*---------------------------- Enable Timer 3 ------------------------------*/
	TIM3->CR1 |= 0x01; //Enable the Timer 3
}
...
{	
	...
	TIM4->SMCR &= ~0x07; //Slave Mode Selection: SMS 000: Counter is directly clocked by CLK_INT
  ...
  TIM4->DIER  |= 0x011; //Enable Update Interrupt, Enable the CC4 Interrupt Request
	TIM4->SR &= ~0x01; //Pending-Bit loeschen
  NVIC_init(TIM4_IRQn,1); //Enable Timer 4 Interrupt
	
	/*---------------------------- Enable Timer 4 ------------------------------*/
	TIM4->CR1 |= 0x01; //Enable the Timer 4
}
...
int main(void) 
{
	...
	TIM3_Config(); //Timer 3 config, fuer Output Capture: Motoransteuerung
	TIM4_Config(); //Timer 4 config, fuer Input Capture: Drezahl
