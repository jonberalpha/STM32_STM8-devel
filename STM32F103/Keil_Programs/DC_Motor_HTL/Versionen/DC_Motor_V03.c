...
{ 
  if(((TIM4->SR & TIM_IT_CC2) != 0) && ((TIM4->DIER & TIM_IT_CC2) != 0))		// Capture Interrupt von Channel 4?
	{
   TIM4->SR &= ~TIM_IT_CC2; /* Clear TIM4 Capture Interrupt pending bit for Channel 4*/
   if(CaptureNumber == 0)
   {      
      IC2ReadValue1 = TIM4->CCR2;/* Get the Input Capture value for Channel 4*/
      CaptureNumber = 1;
   }
...
{	
  ...
  TIM4->CR1 = tmp;
	...
	/* -------------- Configure Channel CH4(PB9) as Input Capture-Channel  (Capture Compare mode Register 1) ---------------*/
  tmp = TIM4->CCMR2;
