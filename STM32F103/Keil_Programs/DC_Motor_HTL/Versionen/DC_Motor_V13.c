...
void TIM1_Config(void); //Timer 1 config
void TIM3_Config(void); //Timer 3 config
...
/******************************************************************************/
void TIM1_UP_IRQHandler(void)	//Timer 1, loest alle 1000ms aus
{
...
void init_ports(void)
{
	...
	GPIOB->CRH |= 0x30000040; //PB15 auf GPOPP; PB9 auf IF
}
...
int main(void) 
{
	...
	//Uhrenanzeige
	int lcd_sekunden=0,lcd_minuten=0,lcd_stunden=0,lcd_zehntel=0;
	char einh[10],ein[10],zeh[10],hour[10],min[10],eimin[10],sek[10];
