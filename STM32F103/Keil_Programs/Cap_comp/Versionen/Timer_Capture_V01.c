...
{ 
  if (((TIM2->SR & TIM_IT_Update) != 0) && ((TIM2->DIER & TIM_IT_Update) != 0))		// Update Interrupt Pending ?
  {
    TIM2->SR &= ~TIM_IT_Update; /* Clear TIM2 Update Interrupt pending bit */
	  LED7=~LED7;
  }
  if (((TIM2->SR & TIM_IT_CC2) != 0) && ((TIM2->DIER & TIM_IT_CC2) != 0))		// Capture Interrupt von Channel 2?
	{
   TIM2->SR &= ~TIM_IT_CC2; /* Clear TIM2 Capture Interrupt pending bit for Channel 2*/
   if(CaptureNumber == 0)
   {      
...
uint16_t tmp = 0;
  ...
  TIM2->CR1 = tmp;
	...
	TIM2->SMCR = 0x0000; 	//Slave Mode Selection: SMS 000: Counter is directly clocked by CLK_INT

	TIM2->PSC = 7936;		    //Value Prescaler (reduce system clock rate to 1KHz
  ...
  NVIC_init(TIM2_IRQn,1);        /* Enable Timer 2 Interrupt  */
