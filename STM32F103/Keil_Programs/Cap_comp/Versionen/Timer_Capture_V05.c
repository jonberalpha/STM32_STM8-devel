...
/*   vectored Interrupt controller (Prioritaet setzen, Auslösen verhindern,   */
/*   Interrupt enablen)                                                       */
...
{ 
  ...
  if (((TIM4->SR & TIM_IT_CC4) != 0) && ((TIM4->DIER & TIM_IT_CC4) != 0))		// Capture Interrupt von Channel 2?
	{
   TIM4->SR &= ~TIM_IT_CC4; /* Clear TIM4 Capture Interrupt pending bit for Channel 2*/
   if(CaptureNumber == 0)
   {      
      IC4ReadValue1 = TIM4->CCR4;/* Get the Input Capture value for Channel 2*/
      CaptureNumber = 1;
   }
...
{	
  ...
  tmp = TIM4->CCMR2;
  tmp &= (~TIM_CCMR2_CC4S) & (~TIM_CCMR2_IC4F); /* sampling of TI2 (Input Capture channel 2) at full speed, not filter */
  ...
  TIM4->CCER &= ~TIM_CCER_CC4E;   /* Disable the Channel 2: Reset the CC2E Bit */
  tmp = TIM4->CCER;
  tmp &= ~TIM_CCER_CC4P; /* Set Polarity input capture: non-inverted: capture is done on a rising edge of IC2 */
  tmp |= (TIM_ICPolarity_Rising << 4) | TIM_CCER_CC4E;  /* Enable Input Capture Channel 2 */
  TIM4->CCER = tmp;      /* Write to TIM4 CCER registers */
	
	/* -------------- Enable Interrupts for Timer 4 -------------------------*/	
  TIM4->DIER  |= TIM_IT_Update;   /* Enable Update Interrupt */
  TIM4->DIER  |= TIM_IT_CC4;     /* Enable the CC2 Interrupt Request */
...
int main (void) 
{
	...
	do
	{
		...
		//Differenz
		lcd_set_cursor(1,8);
		sprintf (&buffer[0], "%d",IC4ReadValue2-IC4ReadValue1); 
		lcd_put_string(&buffer[0]);	
	}
	while(1);
