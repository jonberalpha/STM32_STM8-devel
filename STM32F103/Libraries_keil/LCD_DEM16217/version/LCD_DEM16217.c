...
/******************************************************************************/
#define LCD_DEM16217_MOD
#include "LCD_DEM16217.h"

/*_______________________ FUNCTION PROTOTYPES ________________________________*/
void lcd16x2_toggle_e(void);
void lcd16x2_write(uint8_t data, uint8_t rs);
uint8_t lcd16x2_read(uint8_t rs);
uint8_t lcd16x2_wait_busy(void);
void lcd16x2_new_line(uint8_t pos);

uint8_t display_cursor_on_off_control;
...
void lcd16x2_init(uint8_t disp_attr)
{
  ...
  GPIO_InitStruct.GPIO_Pin = LCD16X2_PIN_D4 | LCD16X2_PIN_D5 | LCD16X2_PIN_D6 | LCD16X2_PIN_D7;
  GPIO_InitStruct.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_InitStruct.GPIO_Speed = GPIO_Speed_2MHz;
  GPIO_Init(LCD16X2_GPIO_DATA, &GPIO_InitStruct);

  // Delay power on
  wait_us(LCD16X2_DELAY_POWER_ON);
  ...
  wait_us(LCD16X2_DELAY_INIT);

  // Repeat last command
  lcd16x2_toggle_e();
  // Delay, busy flag can't be checked here
  wait_us(LCD16X2_DELAY_INIT_REP);
  ...
  lcd16x2_toggle_e();
  wait_us(LCD16X2_DELAY_INIT_4BIT);

  /* From now the LCD only accepts 4 bit I/O */

  // 4-bit interface, 2 lines, 5x7 dot format font
  lcd16x2_write_command(LCD16X2_FUNCTION_SET | LCD16X2_4BIT_INTERFACE | LCD16X2_2LINE_MODE | LCD16X2_5X7DOT_FORMAT);
  // Display off
  ...
  lcd16x2_write_command(LCD16X2_DISPLAY_CURSOR_ON_OFF | display_cursor_on_off_control);
}
...
static void lcd16x2_toggle_e()
{
  ...
  // Pulse length in us
  wait_us(LCD16X2_DELAY_ENABLE_PULSE);
  // EN pin = LOW
...
static uint8_t lcd16x2_wait_busy()
{
  ...
  wait_us(LCD16X2_DELAY_BUSY_FLAG);
